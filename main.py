...
# Load configuration and standard paths
CONFIG = load_config()
...
def stage1_data_generation():
    ...
    # Save to CSV
    filepath = os.path.join(DATA_DIR, 'gold_dataset.csv')
...
def stage2_llm_simulation():
    ...
    gold_filepath = os.path.join(DATA_DIR, 'gold_dataset.csv')
    ...
    annotations = simulate_llm_labeling(dataset, noise_rate=CONFIG.get('labeling', {}).get('llm_noise_rate', 0.15))
    
    # Save LLM annotations
    llm_filepath = os.path.join(DATA_DIR, 'llm_annotations.csv')
...
def stage3_rule_based_checks(annotations_df):
    ...
    review_filepath = os.path.join(OUTPUTS_DIR, 'items_for_review.csv')
...
def stage5_human_corrections(annotations_df, flagged_items):
    ...
    final_filepath = os.path.join(OUTPUTS_DIR, 'final_labeled_after_human.csv')
...
def generate_label_comparison_chart(annotations_df, corrected_df):
    ...
    # Save the plot
    chart_filepath = os.path.join(OUTPUTS_DIR, 'label_comparison.png')
...
def stage6_evaluation_and_visualization(annotations_df, corrected_df, flagged_items):
    ...
    metrics_filepath = os.path.join(OUTPUTS_DIR, 'metrics_summary.txt')
...
def main():
    ...
    print(f"  - {os.path.join(DATA_DIR, 'gold_dataset.csv')} (original dataset with true labels)")
    print(f"  - {os.path.join(DATA_DIR, 'llm_annotations.csv')} (dataset with LLM predictions)")
    print(f"  - {os.path.join(OUTPUTS_DIR, 'items_for_review.csv')} (flagged items for human review)")
    print(f"  - {os.path.join(OUTPUTS_DIR, 'final_labeled_after_human.csv')} (final corrected dataset)")
    print(f"  - {os.path.join(OUTPUTS_DIR, 'label_comparison.png')} (visualization charts)")
    print(f"  - {os.path.join(OUTPUTS_DIR, 'metrics_summary.txt')} (evaluation metrics)")
    ...
    return {
        ...
        'files_generated': [
            os.path.join(DATA_DIR, 'gold_dataset.csv'),
            os.path.join(DATA_DIR, 'llm_annotations.csv'), 
            os.path.join(OUTPUTS_DIR, 'items_for_review.csv'),
            os.path.join(OUTPUTS_DIR, 'final_labeled_after_human.csv'),
            os.path.join(OUTPUTS_DIR, 'label_comparison.png'),
            os.path.join(OUTPUTS_DIR, 'metrics_summary.txt')
        ]
    }
